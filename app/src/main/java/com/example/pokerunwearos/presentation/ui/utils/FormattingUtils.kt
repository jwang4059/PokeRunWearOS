...
fun formatElapsedTime(elapsedDurationLong: Long, includeSeconds: Boolean) = buildSpannedString {
    val elapsedDuration = elapsedDurationLong.toDuration(DurationUnit.MILLISECONDS)
    val hours = elapsedDuration.inWholeHours
    if (hours > 0) {
        append(hours.toString())
        inSpans(RelativeSizeSpan(UNITS_RELATIVE_SIZE)) {
